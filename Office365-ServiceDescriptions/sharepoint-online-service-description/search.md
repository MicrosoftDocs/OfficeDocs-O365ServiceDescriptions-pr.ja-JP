...
検索システムは、コンテンツの接続方法、検索結果におけるアイテムの出現頻度、ユーザーがどの検索結果をクリックしたかによって検索結果の関連性を判断します。分析コンポーネントがこの情報を追跡および分析し、それを使用して継続的に関連性を改善します。詳細については、「[SharePoint Server 2013 の分析処理の概要](https://docs.microsoft.com/SharePoint/search/overview-of-analytics-processing)」を参照してください。
...
ユーザーは、関連性、新しさ、社会的距離 (人の名前) などのさまざまな条件で検索結果を並べ替えることができます。詳細については、「[検索結果 Web パーツの設定を変更する](https://support.office.com/article/change-settings-for-the-search-results-web-part-40ff85b3-bc5e-4230-b1dd-f088188e487e)」を参照してください。
