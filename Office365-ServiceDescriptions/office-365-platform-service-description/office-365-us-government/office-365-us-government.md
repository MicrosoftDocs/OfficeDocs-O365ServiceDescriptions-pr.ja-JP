...
> [!IMPORTANT]
> Microsoft Teams では、コロナウイルス (COVID-19) パンデミックにより、オンライン通話と音声/ビデオ会議が急増しています。<br/>
>
>Microsoft は、通話の前例のない増加に対応し、継続性と可用性を確保するために、Microsoft Teams GCC オーディオ/ビデオ サーバーが商用データセンターおよび政府機関データセンターの処理能力を活用できるようにしています。<br/>
>
>これらのオーディオ/ビデオ サーバーは、米国の Microsoft Azure FedRAMP High 認定境界サーバー内に存在し、顧客コンテンツを保存しません。 ただし、これらのサーバーは通話や会議のオーディオとビデオを処理し、この中間期間中は商用スタッフの下で動作しています。<br/>
>
>修飾された、スクリーンされた担当者は、これらのサーバーに対する対話型ログオンを確認することにより、これらのサーバーを監視して、顧客データへの潜在的なアクセスを監視しています。 資格のある担当者は、顧客コンテンツへのアクセスに関する GCC 要件を満たします。 スクリーニング要件の詳細については [、「GCC サービスの説明」を参照してください](gcc.md)。<br/>
>
>これらの特別な時代にサービスが利用可能で信頼性が高い状態を維持するための手順を実行する場合は、サポートに感謝します。<br/>

米国の公共部門の独自の要件と進化する要件に応じて、Microsoft は Office 365 Government プラン (または 365 Government Office) を作成しました。 このサービスの説明では、365 Government US 環境に固有Office概要を示します。 このサービスの説明は、他の Microsoft [365 および 365](../../office-365-service-descriptions-technet-library.md)サービスOfficeと共にお読みすることをお勧めします。

## <a name="how-to-use-this-service-description"></a>このサービスの説明を使用する方法

365 Officeサービスの説明は、365 サービスの一般的な説明へのオーバーレイOffice設計されています。 Office 365 Enterprise 製品と比較した独自のコミットメントと相違点を定義します。

## <a name="about-office-365-government-environments"></a>365 Office環境について

Office 365 政府機関プランは、毎月のサブスクリプションであり、無制限の数のユーザーにライセンスを取得できます。

- この **Office 365 GCC** 環境は、FedRAMP High を含むクラウド サービスに関する連邦要件、および刑事司法および連邦税情報システム (CJI および FTI データ型) の要件に準拠しています。

- **Office 365 GCC High および DoD** 環境は、米国国防総省のセキュリティ要件のガイドライン、国防総省調達規則 (DFARS)、武器国際取引に関する規制 (ITAR) に準拠します。

Office 365 の機能に加えて、Office 365 Government を使用する組織には、Office 365 Government 固有の機能があります。
...
Office 365 Government は、(1) 米国の連邦、州、地方、部族、および地域の政府機関、および (2) 政府の規制および要件の対象となるデータを処理するその他のエンティティ、および Office 365 Government の使用がこれらの要件を満たすのに適しています。適格性の検証の対象となります。 Microsoft による適格性の検証には、武器国際取引に関する規制 (ITAR) の対象となるデータ、FBI の米国刑事司法情報サービス (CJIS) ポリシーの対象となる法律施行データ、またはその他の政府が規制または制御するデータの取り扱いに関する検証が含まれます。 検証には、ITAR データに関する米国国務省への登録の証明や、データの取り扱いに関する特定の要件についての政府機関による公的支援が必要な場合があります。 このOffice 365 DoD 環境は、米国国防総省の排他的な使用用です。

365 政府機関向Office条件は一貫しますが、Microsoft は GCC High 環境の DFARS および ITAR 契約言語にのみ同意します。

365 Government の適格性に関する質問があるエンティティOfficeアカウント チームに問い合わせるべきです。

365 Government に対する顧客の契約Office、適格性の再評価が必要です。

## <a name="customer-content-located-within-the-united-states"></a>米国内の顧客コンテンツ

Office 365 政府機関サービスは、米国に物理的に配置されたデータセンターから提供されます。 次の顧客コンテンツは、米国に配置されたデータ センターにのみ格納されます。

- Exchange Online メールボックス のコンテンツ (電子メールのボディ、予定表エントリ、および電子メール添付ファイルのコンテンツ)

- SharePoint Online サイトのコンテンツと、そのサイト内に格納されているファイル

- Skype for Business アーカイブされた会話、アップロードされたドキュメント、ホワイトボード セッション

- Microsoft Teams 常設チャット スレッド
...
SharePoint サイトの [サイトの利用状況] ページは政府機関のプランで利用できます。コンプライアンスごとに、このページの一部の機能は商用ユーザーだけが利用できます。 詳細については [、「Microsoft 365](https://support.microsoft.com/office/2fa8ddc2-c4b3-4268-8d26-a772dc55779e)の SharePoint サイトのサイト使用状況ページ」を参照してください。

## <a name="office-365-government-and-third-party-services"></a>Office 365 Government およびサード パーティのサービス

Office 365 では、Microsoft 365 Apps for enterprise (Word、Excel、PowerPoint、Outlook など) に含まれる SharePoint Online サイト、Skype for Business、Office アプリケーション、および Outlook Web App にサードパーティ アプリケーションを統合できます。 さらに、Office 365 はサード パーティ サービス プロバイダーとの統合をサポートしています。 これらのサード パーティ アプリケーションおよびサービスには、組織の顧客データを Office 365 インフラストラクチャ外部のサード パーティ システムで保管、送信、処理する場合があるため、Office 365 のコンプライアンスおよびデータ保護に関するコミットメントは適用されません。 組織でサード パーティのサービスを使用することが適切であるかどうか査定する際には、そのサード パーティのプライバシーおよびコンプライアンスに関する声明を検討することをお勧めします。

## <a name="restricted-data-access-by-administrators"></a>管理者に制限されたデータ アクセス

Microsoft 管理者Office 365 政府機関の顧客コンテンツへのアクセスは、スクリーン担当者に制限されます。 スクリーニング レベルの詳細については、各環境 (GCC または GCC High および DoD) のサービスの説明ページを参照してください。

## <a name="fasttrack-center-onboarding-assistance"></a>FastTrack Center オンボーディング アシスタンス

fastTrack Center Benefit for Office 365<sup>1</sup>では、FastTrack スペシャリストとリモートで作業し、Office 365 環境を使用できる状態にし、組織内でのロールアウトと使用状況を計画します。 FastTrack プロセスでは、オンボーディングと、ユーザー採用サービスが提供されます。

オンボーディングは、次のもので構成されます。

- コア オンボーディング - テナントの構成と Azure Active Directory (Azure Active Directory AD) との統合に必要なタスクです。 コア オンボーディングは、他の対象のサービスのオンボーディングのベースラインも提供します。
...
<br/><sup>2</sup> データ移行サービスは、500 Officeライセンスを持つ 365 テナントに対して利用できます。

## <a name="data-migrations-performed-by-fasttrack"></a>FastTrack によって実行されるデータ移行

[FastTrack](https://fasttrack.microsoft.com/)移行特典を選択したお客様は、データ移行を管理するチームにアクセス権を付与する必要があります。 これらの担当者は米国市民であり、365 US Government サービスの顧客の移行を実行する前に、Officeのバックグラウンド チェックを受ける必要があります。<br><br>
...
Office 365 米国政府機関のお客様は、Azure Government ExpressRoute サービスを使用して、パブリック インターネットを使用して接続する代わりに、サポートされている Office 365 サービスにプライベートに接続できます。

サポートされているプロバイダー、価格モデルなど、詳細については、Azure ExpressRoute の情報 [を確認してください](/azure/expressroute/)。

Azure ExpressRoute Office 365 のサポートの詳細については [、「Azure ExpressRoute for Office 365」を参照してください。](/microsoft-365/enterprise/azure-expressroute)
...
|Voice - 電話システム、電話会議|いいえ<sup>2</sup>|いいえ<sup>2</sup>|はい<sup>5</sup>|いいえ|
...
|Yammer エンタープライズ|いいえ<sup>4</sup>|いいえ<sup>4</sup>|いいえ<sup>4</sup>|いいえ<sup>4</sup>|

> <sup>1</sup> Skype for Business Basic は、すべてのお客様が利用できます。 Skype for Business デスクトップ クライアントは、Skype for Business Online を含む Office 365 プランのプレゼンス機能、インスタント メッセージング機能、会議機能を提供するローカルにインストールされるアプリケーションです。 Microsoft 365 Apps for enterprise、G3、G5 には、高度なテレフォニー サポート、アーカイブ、コンプライアンス機能などの追加機能を含む完全な Skype アプリケーションが含まれています。 Skype for Business Online のライセンスは、ユーザーごとに割り当てる必要があります。
<br/><sup>2</sup> 含まれていませんが、個別のアドオンとして購入できます。 Project Online には、サブスクリプションの一部として Project Online デスクトップ クライアントが含まれています。
<br/> <sup>3</sup> GCC High プランまたは DoD プランではまだ利用できませんが、近日公開予定です。
<br/><sup>4</sup> Yammer Enterprise は Office 365 US Government のコンポーネントではないが、GCC の Office 365 のライセンスを持つ各ユーザーのスタンドアロン オファーとして無料で取得できます。 この提供は現在、エンタープライズ契約およびエンタープライズ サブスクリプション契約に基づいて Office 365 GCC を購入するお客様に限定されています。 Yammer は、GCC High または DoD では利用可能ではありません。
<br/><sup>5</sup> 通話プランはアドオンです。
<br/><sup>6</sup> 含まれていませんが、個別のアドオンとして購入できます。 Visio for the web には、サブスクリプションの一部として Visio デスクトップ アプリが含まれています。
...
|---|---|---|---|---|
|**Office 365 管理**|||||
|Microsoft 365 管理センターを使用して管理Office 365|は<sup>い 16</sup>|は<sup>い 16</sup>|はい|は<sup>い 16</sup>|
...
|Microsoft 365 管理センター ダッシュボードで利用できる個々のアラートの状態|は<sup>い 9、15</sup>|は<sup>い 9、15</sup>|は<sup>い 9、15</sup>|は<sup>い 9、15</sup>|
...
|Microsoft 365 レポート Web サービスを使用して独自のレポートを作成する|は<sup>い 15</sup>|は<sup>い 15</sup>|は<sup>い 15</sup>|は<sup>い 15</sup>|
...
> <sup>1</sup> Azure Information Protection は含まれていませんが、個別のアドオンとして購入できます。Azure Information Protection は、サポートされている Information Rights Management (IRM) 機能を有効にします。 Azure Information Protection の一部の機能では、Microsoft 365 Apps for enterprise へのサブスクリプションが必要ですが、Office 365 Government G1 または Office 365 Government F3 には含まれません。 >
<br/><sup>2</sup> 既存の BBCS および BIS のお客様は、サービスを引き続き使用できます。 新規のお客様の受け付けはしていません。
<br/><sup>3</sup> ディレクトリ同期を使用する場合は、Office 365 ポータルではなく Active Directory を使用するか、または Windows PowerShell の Azure Active Directory モジュールを使用してアカウントを削除するか、パスワードを変更する必要があります。
...
<br/><sup>6</sup> 以前のバージョンの Office 365 からアップグレードしない限り、Office 365 のパブリック web サイトを 1 Officeできます。 この場合、パブリック Web サイトが 2 つあっても、どちらか一方のサイトしかカスタム ドメイン名でホストすることができません。 2 つの Web サイト for Business サブスクリプションの操作の詳細については、「Work with your 2 Office [365 パブリック Web サイト」を参照してください](https://go.microsoft.com/fwlink/p/?LinkID=271589)。 別のサブスクリプションをお持ちの場合は、「パートナー Web サイトホスティングとパブリック Web サイトの詳細」を参照Office [365.](https://go.microsoft.com/fwlink/p/?LinkID=325009)
<br/><sup>7</sup> 期間割引で購入したシートを減らすには、早期終了料金が適用される場合があります。 これは月単位で支払われるサブスクリプションには適用されません。
<br/><sup>8</sup> 次のプランでは、Microsoft 365 管理センターからのライセンス シートの変更はサポートされていません。Office 365 Government G1、Office 365 Government G3、Office 365 Government F3)。
<br/><sup>9</sup> GCC High ではまだ利用できませんが、近日公開予定です。
<br/><sup>10</sup> Office 365 Government G1、G3、F3 の場合は、ビジネス ロードマップの対象Office 365 が適用されます。ただし、コンプライアンス要件により、特定のサービス更新プログラムにいくつかの違いや遅延 [が生じ得る場合があります](https://www.microsoft.com/trust-center)。
<br/><sup>11</sup> 365 Government Officeではまだ利用できませんが、近日公開予定です。
<br/><sup>12</sup> 含まれていませんが、GCC で個別のアドオンとして購入できます。
<br/><sup>13 365</sup> 政府機関向Officeサポートされていません。
...
<br/><sup>16 管理</sup> センターには、DoD 環境または GCC High 環境での使用状況分析は含めされません。
<br/><sup>17</sup> GCC High または DoD 環境ではサポートされていません。
<br/><sup>18</sup> GCC High および DoD では、ユーザーとドメインの偽装およびスプーフィング インテリジェンスに対するフィッシング対策はまだ利用できません。
<br/><sup>19</sup> GCC 環境ではまだ使用できませんが、近日公開予定です。
...
|iOS および Android<sup>5、4 の Outlook</sup>|はい|はい|はい|はい|
...
<br/><sup>3</sup> Skype for Business Basic は、すべてのお客様が利用できます。 Skype for Business デスクトップ クライアントは、Skype for Business Online を含む Office 365 プランのプレゼンス機能、インスタント メッセージング機能、会議機能を提供するローカルにインストールされるアプリケーションです。 Microsoft 365 Apps for enterprise、Office 365 Enterprise E3 には、高度なテレフォニー サポート、アーカイブ、コンプライアンス機能などの追加機能を含む完全な Skype アプリケーションが含まれています。 A Skype for Business Online license must be assigned for each user. For more information on Lync Basic features, see [Skype for Business Online client comparison tables](/lyncserver/lync-server-2013-desktop-client-comparison-tables).
<br/><sup>4</sup> GCC High または DoD 環境ではまだ使用できませんが、近日公開予定です。
<br/><sup>5 詳細</sup> については [、「Government Community Cloud での Outlook for iOS](/exchange/clients-and-mobile-in-exchange-online/outlook-for-ios-and-android/outlook-for-ios-and-android-in-the-government-cloud) と Android の使用」を参照してください。
<br/><sup>6</sup> 365 DoD 環境Officeまだ使用できませんが、近日公開予定です。
...
|---|---|---|---|
|[**Microsoft Excel**](../../office-applications-service-description/office-applications.md#microsoft-excel)は、現時点では使用できない次の機能を除き、政府機関のクラウドで完全に利用できます。||||
|3D 埋め込みアニメーションと 3D モデル|いいえ|いいえ|いいえ|
|データ型|いいえ|いいえ|いいえ|
|フラッシュ フィル|いいえ|いいえ|いいえ|
|Ideas (Insight サービス)|いいえ|いいえ|いいえ|
|Power BI との統合の強化 (カスタム ビジュアル、Excel から直接 PBI グラフを作成する)|いいえ|いいえ|いいえ|
...
|グラフ: マップ|いいえ|いいえ|いいえ|
|Intelligent Digital Ink|いいえ|いいえ|いいえ|
|PowerPoint でのライブキャプションと字幕|いいえ|いいえ|いいえ|
...
> <sup>1</sup> 今後の可用性。
<br/><sup>2</sup> ローカル Surface Hub の可用性 (サインインしていない)。
<br/><sup>3</sup> GCC 環境で外部共有を利用できます。 組織の Microsoft フォームをオフまたは [有効にする方法の詳細](https://support.office.com/article/cc52287a-4550-464d-9a1b-457bf9df2240#PickTab=Configure) については、以下を参照してください。 GCC High 環境と DOD 環境では、外部共有が無効になります。組織内のユーザーは、フォームに入力して応答を送信し、フォームをテンプレート[](https://support.office.com/article/82ea9d8a-260a-47a0-afdb-497f3d746e3f)として複製して共有し、フォームを共同[](https://support.office.com/article/d5bb5cf0-8401-4c15-bb8c-8e108cd7e69b)編集または共同作業し、フォームの結果に[アクセス](https://support.office.com/article/02859424-341d-406f-b32a-9a0fbaf357af)できます。
<br/><sup>4</sup> GCC 環境では、最近使用したグループ フォーム機能が無効になっています。 ただし、ユーザーは[グループ フォーム] タブで特定のグループを選択することで、グループ フォームにアクセスできます。
<br/><sup>5</sup> Word、Excel PowerPoint Windows クライアントのみ、Web、MacOS、iOS、Android ではありません。