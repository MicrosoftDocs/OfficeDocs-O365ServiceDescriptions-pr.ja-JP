...
|ロードマップ | Microsoft Project Online、Microsoft Project for the web、Azure Boards プロジェクトのプロジェクト間で、視覚的でインタラクティブなロードマップを作成します。<sup>2</sup> Project Plan 1 を使用すると、ロードマップへの読み取り専用アクセスが可能になります。 Project Plan 3 または Project Plan 5 では、ロードマップを作成することもできます。 | 表示のみ | 表示のみ | はい | はい |
|**レポート**<sup>3</sup> ||||||
|すぐに使えるレポート | 事前に作成されたレポートを使用して、プロジェクト、プログラム、ポートフォリオ、およびリソース データを理解します。 | 不要 | はい | はい | はい |
|カスタム レポート | すぐに使用できるレポートに基づいて、またはゼロからカスタム レポートを作成して、組織にとって重要なデータを理解します。 | いいえ | はい | はい | はい |
|ダッシュボードとポータル | 特定の組織のニーズを満たすのに役立つカスタム ダッシュボードとポータルを構築します。| いいえ | はい | はい | はい |
|**使いやすさ** ||||||
...
|**カスタマイズと統合**||||||
|PowerApps | PowerApps を使用して、Web およびモバイル向けのソリューションを構築および利用します。<sup>4</sup> | いいえ | いいえ | はい | はい |
|ワークフロー | Power Automate を使用してビジネス プロセスを自動化および統合します。<sup>5</sup> | 不要 | いいえ | はい | はい |
|Microsoft 統合 | 使い慣れた Microsoft のツールとアプリケーションを使用して、ドキュメントの共同作業や管理などを行います。<sup>1</sup> | はい | はい | はい | はい |
|**セキュリティとユーザー管理**||||||
...
|リソース管理 (予約) | プロジェクト マネージャーは、役割、スキル、または名前でリソースの要求を送信できます。 リソース マネージャーは要求を確認し、利用可能な最良のリソースを割り当てることができます。 |はい<sup>7</sup> | はい<sup>8</sup> |
...
|ワークフロー | Power Automate を使用して (ODataを介して) ビジネス プロセスを自動化および統合します。<sup>5</sup> | はい | はい |
...
|**セキュリティ、ユーザー、サービス管理**||||
|Active Directory の統合 | 組織の Active Directory のメンバーからチームを構築します。<sup>9</sup> | はい | はい |
...
<sup>9</sup> Active Directory には別のサブスクリプションが必要です。 </br>
