...
|メールボックス フォルダーごとのサブフォルダー数の最大値|1万<sup>2</sup>|1万<sup>2</sup>|1万<sup>2</sup>|1万<sup>2</sup>|1万<sup>2</sup>|1万<sup>2</sup>|
...
> <sup>2</sup> これはストアの制限です。これは、メールボックス図形の制約の1つです。 指定した親に対しては、1万の直接の子フォルダーが存在することができます。 これは、移行またはフォルダーを作成するその他のクライアントに関係なく適用されます。
...
> [!NOTE]
> <sup>1</sup> Microsoft メールボックスの既定の最大メッセージサイズは 25 MB です。 Microsoft 管理者は、1 MB と 150 MB の間でカスタムの制限を指定できます。 ただし、送信または受信できるメッセージのサイズは、電子メール クライアントまたはソリューション サポートにも依存します。 組織で許可される最大メッセージサイズのカスタマイズの詳細については、「 [Microsoft が大規模なメールメッセージをサポート](https://go.microsoft.com/fwlink/?linkid=2144144)する」を参照してください。 
...
> [!NOTE]
> <sup>1</sup> Microsoft 管理者は、1 mb と 150 MB の間でカスタムの制限を指定できます。 ただし、送信または受信できるメッセージのサイズは、電子メール クライアントまたはソリューション サポートにも依存します。 組織で許可される最大メッセージサイズのカスタマイズの詳細については、「 [Microsoft が大規模なメールメッセージをサポート](https://go.microsoft.com/fwlink/?linkid=2144144)する」を参照してください。 <br/> <sup>2</sup> ユーザー間で最大 150 MB のメッセージを送受信できます (メッセージが Microsoft データセンターから送信されることはありません)。 Microsoft データセンターの外部にルーティングされるメッセージには、33% の翻訳エンコードの増加が追加されます。この場合、最大メッセージサイズは 112 MB になります。 <br/> 
...
> [!NOTE]
> <sup>1</sup> メールボックスが Exchange Online に移行された場合、受信トレイルールの制限は、既定の exo 値より小さい値に設定される場合があります。 その場合は、受信トレイルールの値を増やすことができます。 手順については、「 [Exchange Online での受信トレイルールで使用されるスペースを変更](https://docs.microsoft.com/exchange/clients-and-mobile-in-exchange-online/outlook-on-the-web/increase-the-space-used-by-inbox-rules)する」を参照してください。 
